...
% (adapted from http://vis.berkeley.edu/courses/cs294-69-fa11/wiki/index.php/CS294-69_Image_Manipulation_and_Computational_Photography)
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (4) Vectorization for accelerating the computations
% Warning:
...
I = imread('text1.jpg');        % Read a PNG image, the image is a uint8 image (range [0,255])

figure
imshow(I)                     % Display the image

size(I)                       % Retruns the size of the image

I= im2double(I);              % Transform the image from uint8 to double (range[0,1])

figure
imagesc(I)                    % Display ithe transformed image

I = rgb2gray(I);              % convert RGB image to grayscale image
figure
...
figure;
h= fspecial('laplacian')% fspecial can be used to create special type of filters, 
                              % such as gaussian, laplacian etc.
I2= imfilter(I,h);            % applies the h filter to the image
